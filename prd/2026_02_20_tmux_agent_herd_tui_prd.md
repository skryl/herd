# Herd TUI PRD (tmux Agent Herding)
...
1. Build a local CLI/TUI tool (`herd`) that discovers tmux sessions and surfaces agent status in real time.
...
1. Create module layout and root command (`herd tui`, `herd sessions`, `herd herd` placeholder).
...
3. Expose status summary in `herd sessions`.
...
1. `herd tui` opens a split screen with sessions/status left and selected content right.
...
1. `cargo test`
   - result: `pass`
   - notes: full phase test suite green (phase0..phase5 tests).
2. `cargo test --lib`
   - result: `pass`
   - notes: library-target unit gate passes.
3. `cargo test --test '*'`
   - result: `pass`
   - notes: integration/boundary test gate passes.
4. `cargo build` + isolated tmux smoke session launching `herd tui` and quitting with `q`
   - result: `pass`
   - notes: TUI renders split layout and exits cleanly in runtime smoke.
5. `~/.asdf/installs/rust/1.93.1/bin/cargo-clippy --all-targets --all-features`
   - result: `pass`
   - notes: clippy gate passes using active asdf toolchain binary.
6. `~/.asdf/installs/rust/1.93.1/bin/cargo-fmt --all -- --check`
   - result: `pass`
   - notes: formatting gate passes after applying rustfmt.
7. `cargo-clippy --all-targets --all-features` and `cargo-fmt --all -- --check`
   - result: `pass`
   - notes: shimmed formatter/linter commands available and green in current shell.
